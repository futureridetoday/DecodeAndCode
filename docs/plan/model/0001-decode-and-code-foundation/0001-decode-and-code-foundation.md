...
| D-28 | **O `huddle.md` deste repositório não viaja; o mecanismo que cria um vazio, sim** | **Decidida na derivação da Fase 7, em 2026-08-26, e ela precisa de uma frase deste plano.** A seção *O `huddle` é fila, não fonte* afirma que *"o huddle viaja no plugin, a rotina não"*, e a leitura literal empacotaria `docs/plan/system/huddle.md` — que hoje carrega **nove entradas abertas** que são a conversa entre o humano e o modelo **deste** projeto: a `H-01` sobre a ordem das fases daqui, a `H-06` sobre um agente do AmFlow, a `H-09` sobre um hábito meu. Instância pura, pelo invariante 2, e o humano já declarou em 2026-08-26 que o huddle é a conversa dele e não registro de revisão de projeto. **O que a frase do plano protege continua de pé:** o huddle é parte do método, ao contrário do agendamento da conversa, que é configuração de máquina. O que viaja é `huddle.iniciar`, que cria o esqueleto vazio no projeto que instala — e **não há arquivo de template**, pela `D-20`: o formato vive no script, como `porte._CONTEUDO_INICIAL` já faz |
...
| L-36 | **Um teste afirmava o estado do plano real, e a última unidade do escopo o quebrou** | **Aberta e corrigida em 2026-08-27, na revisão da `21`.** `test_plano_real_0001_projeta_sem_levantar_e_sem_regredir` afirmava `situacao == "em desenvolvimento"` sobre o plano **real** — e quebrou quando a `0001-21` fechou o plano de verdade. **Nenhuma das 21 unidades previu isso**, e não por descuido: o caso existe pela lição da `L-31` — testar contra a instância viva, não só contra fixture —, e essa lição não vem com a segunda metade, que é **separar, dentro da instância, o que é invariante do que é estado do momento**. A situação de um plano é estado: era `em desenvolvimento`, virou `concluído`, e volta se uma unidade de correção entrar. **A primeira correção movia a fragilidade em vez de removê-la** — trocar o literal por `"concluído"` mantém o acoplamento e adia a quebra. Corrigido afirmando que a situação é **um dos dois valores válidos**: o que o caso existe para provar é que a projeção do plano real não levanta e produz o backlog, e a lógica da situação já está coberta contra planos sintéticos em `test_situacao.py`. **O que fica em aberto:** nada distingue, hoje, um caso contra instância que afirma invariante de um que afirma estado — e a diferença só aparece quando o estado muda |
...
| [0001-21](21-huddle-log.md) | huddle-log | `verified` |

21 de 21 derivadas · 21 verificadas · atualizado em 2026-08-27
<!-- backlog:end -->
