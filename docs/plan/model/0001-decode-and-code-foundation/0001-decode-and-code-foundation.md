...
| L-17 | **O modo `derive` não é re-executável, e a `D-12` supõe que seja** | A `D-12` afirma que o derive incremental *"é suportado sem nada novo"* porque `numeracao.proxima_unidade` lê o disco — verdadeiro para a numeração, que devolve `03` corretamente. Mas o passo 1 do modo chama `scaffold.aprovar(plano)` incondicionalmente, e sobre o plano já movido isso levanta `ValueError: nome de plano inválido '0001-decode-and-code-foundation' — começa com prefixo numérico` (medido em 2026-08-24, `dry_run`, sem escrita). Falha ruidosa e antes de qualquer escrita, então não corrompe nada — mas bloqueia a derivação da Fase 2 até o modo ganhar caminho para plano já aprovado. **Fecha antes da Fase 2**, e a correção é da skill, não do plano |
| L-18 | **A situação projetada em `_planos.md` mede as unidades derivadas, não o plano** | Ao fechar a `0001-02`, `backlog.projetar` levou a situação a `concluído` com quinze unidades ainda por derivar — e o efeito não é cosmético: a região `planos` ficou **sem nenhuma linha `em desenvolvimento`**, que é exatamente o que o check de concorrência do modo `review` varre. Um plano novo em `model`/`decode-and-code` teria passado num check que deveria sinalizá-lo. **Não é ocorrência única:** repete a cada fronteira de fase em que todas as unidades derivadas estão verificadas, e a derivação da Fase 2 em 2026-08-24 a reverteu para `em desenvolvimento` sem ninguém editar projeção. É a mesma raiz do custo que a `D-12` aceitou — o rodapé dizendo *"N de N derivadas"* —, mas com consequência funcional que a `D-12` não previu. **Correção candidata:** a situação passa a considerar o escopo declarado do plano, não só o disco |
| L-19 | **O fluxo de decodificação da `03` é derivação, não cópia da fonte** | O plano credita o fluxo `Clarificar → Evitar → Reduzir` e os Gates A/B à guideline `decode_code` de `futureridetoday/CortexMachine`. **O repositório não está em disco** — verificado em 2026-08-24. Escrever a unidade apontando para uma fonte inacessível quebraria o cold-start, então o texto normativo dos três estágios e dos dois gates foi **escrito na própria unidade**, derivado do que este plano descreve e dos três princípios da `D-03`. É suficiente para executar; não é fiel por construção. Se o `CortexMachine` ficar acessível, reconciliar — e a diferença, se houver, é decisão humana, porque a fonte original é de outro domínio e o plano já registra o que dela **não** foi aproveitado |
| L-20 | **O fixture da `04` depende de um repositório externo** | O statement de DDL que escolhe a regra é o incidente `notifications_hub_id_fkey`, registrado no AmFlow — localizado em 2026-08-24 em `docs/plan/_inbox/notification-fk.md` e no gate de encerramento do `0004-close-surface-split`. Daqui é **somente leitura**, e a unidade diz onde ler. Duas consequências ficam ditas: quem executar a `04` sem acesso ao AmFlow não consegue o statement real, e reconstruí-lo de memória produziria um fixture que parece certo e não é — o padrão que a `H-08` registra. Não vira cópia versionada aqui: a regra é instância do AmFlow e o invariante 2 a mantém fora do plugin |
...
5 de 5 derivadas · 2 verificadas · atualizado em 2026-08-24
<!-- backlog:end -->
