...
| D-11 | **A distribuição passa a ser asset de GitHub Release (`source: archive`), revertendo a `D-09`; a `D-10` permanece** | Decidida em **2026-09-01** nesta sessão, aprovada por Bortoli; execução no plano [`_inbox/plugin-release-distribution.md`](../../_inbox/plugin-release-distribution.md). A `D-09` versionou `dist/` porque um `source` de caminho relativo precisa existir no clone do marketplace — mas a doc oficial (`plugin-marketplaces`, *Zip Archive source*) tem `{ "source": "archive", "url", "sha256" }`, que busca o pacote de uma URL HTTPS e **não** exige árvore construída no repositório. O pacote vira zip anexado a um GitHub Release (`v1.0.0` / `decode-and-code-1.0.0.zip`); `dist/` volta ao `.gitignore`; `TestPacoteCommitadoEstaSincronizado` sai; a intenção original da `D-21` é restaurada. A `D-10` fica de pé — raiz só marketplace, manifesto-fonte em `.claude/plugin.json`. Muda junto a identidade: marketplace `name` `bortoli` → `future-ride-today`, `owner` → `Future Ride Today`, e o plugin ganha `displayName: "Decode and Code"` |
