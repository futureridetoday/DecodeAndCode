...
| B-05 | **Auditar as tabelas `## Arquivos` contra o disco, distinguindo criado de removido.** Nenhum gate confere se o que uma unidade declarou tocar corresponde ao que existe: o gate de unidade roda o teste declarado, e `lint_unidade` só exige que a tabela tenha **algum** caminho concreto. As duas ocorrências medidas apontam para o mesmo buraco em direções opostas — a `L-29`, onde a `0001-15` declarou `fixtures.py` e **não o tocou**, e a auditoria de 2026-08-27, onde eu li a `0001-02` como defeituosa por declarar `test_deprecacao.py` que não existe em disco, quando a tabela dizia corretamente `**removido**`. **O invariante é de par, não de existência:** caminho marcado `novo` ou alterado existe; caminho marcado `removido` **não** existe; e o que a tabela não classifica não é verificável. Escrever quando houver terceira ocorrência, ou junto do plano que dividir a norma — não numa data. **Cuidado que a segunda ocorrência ensina:** auditoria montada na hora reprova artefato correto, então este item nasce com a exigência de um caso para cada um dos três estados | feature | — | Claude | model | 2026-08-27 | `lint_plano` / `lint_unidade` |
...
| B-06 | **O primeiro uso do plugin direciona o instalador a configurar a própria norma.** O plugin carrega só o **mecanismo** — `.claude/rules/*` e `.claude/guardrails.json` são do projeto que instala e **não viajam** com ele (README, *O que viaja no plugin*). Hoje nada diz isso a quem instala: para descobrir que precisa escrever os próprios princípios, guidelines e guardrails, o instalador tem que achar a seção *Como usar* do README e os três guias [`create-principle.md`](../../recursos/create-principle.md) / [`create-guideline.md`](../../recursos/create-guideline.md) / [`create-guardrail.md`](../../recursos/create-guardrail.md) por conta própria. A ideia é o **primeiro uso** detectar a ausência de norma de projeto e apontar para os três guias — provável forma: hook de sessão que vê `.claude/rules/` sem guideline de projeto e `guardrails.json` sem regra, ou um comando `/decode-and-code:setup`. **Cuidado:** o hook `InstructionsLoaded` já anuncia norma e guidelines a cada carregamento — este item é *onboarding único*, não mais um anúncio recorrente. **Quando escrever:** junto do primeiro onboarding real de terceiro — a primeira instalação fora de `DecodeAndCode` e `AmFlow`. Até lá não há "primeiro uso" real para calibrar, e o único instalador (AmFlow) já passou sem ele | feature | — | Bortoli | model | 2026-09-03 | hook de sessão + `docs/recursos/create-*.md` |
