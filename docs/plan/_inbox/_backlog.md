...
| B-04 | **Isolar o `git` real de `scripts/move-md.py` nos testes.** Medido em 2026-08-25 com um shim de `git` no `PATH`: a suíte executa **16** `git ls-files --error-unmatch` de verdade, todas de [`move-md.py:196`](../../../scripts/move-md.py), que confere se a origem está versionada antes de usar `git mv`. Não quebra nada e é pré-existente — o registro existe porque foi essa suposição não conferida que tornou falsa a afirmação *"nenhum teste executa `git` de verdade"* da unidade `0001-15` (`L-28`). Mockar o `_versionado` nos testes que movem arquivo resolve; é escopo próprio, não da unidade | problema | — | Claude | model | 2026-08-25 | `scripts/move-md.py` |
